...
cp -a /tmp/kitchen/* $COOKIECUTTER_DIR

# use default values or fail over to example
[[ -e /tmp/kitchen/cookiecutter.json ]] && {
  cp -f /tmp/kitchen/cookiecutter.json  $COOKIECUTTER_DIR/cookiecutter.json
}
[[ ! -e $COOKIECUTTER_DIR/cookiecutter.json ]] && {
  cp -f $COOKIECUTTER_DIR/cookiecutter.json.example $COOKIECUTTER_DIR/cookiecutter.json
}

# generate model and link it as /srv/salt/reclass
cookiecutter $COOKIECUTTER_DIR -o /srv/salt -f --no-input
[[ ! -e /srv/salt/reclass ]] && {
  ln -s /srv/salt/*-salt-model /srv/salt/reclass
}
